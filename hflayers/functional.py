...
def hopfield_core_forward(
    ...
    if not torch.jit.is_scripting():
        ...
        if any(
            ...
            return nn.functional.handle_torch_function(
                ...
                value_as_connected=value_as_connected,
                normalize_pattern=normalize_pattern,
                normalize_pattern_eps=normalize_pattern_eps,
    ...
    update_step, xi_old, xi_difference_norm = 0, None, float(r"+inf")  # noqa
    ...
    while update_active_heads.any():
                ...
                if value_as_static:
                    ...
                    if in_proj_bias is not None:
                        v = nn.functional.linear(
                            v, v_proj_weight_non_opt, in_proj_bias[_start:]
                        )
                    else:
        ...
        if type(scaling) == float:
            ...
            q = q * scaling.view(1, 1, -1).repeat(
                repeats=(1, 1, q.shape[2] // scaling.shape[0])
            )
